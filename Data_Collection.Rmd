...
same <- c(31,33,51,52,53,54,55,56,57,65,70)
NBA23 <- select(NBA23, -same)
head(NBA23)
```
Now, each column represents a different statistic, but we need to rename the variables so they are easier to referenece when we need to access them.
...
The "NBA23" data set contains 65 columns (variables) and 540 rows representing the 540 different players from the 2022-2023 NBA season.
